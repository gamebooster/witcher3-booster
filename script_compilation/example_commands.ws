...
import abstract class CBehTreeMachine {
  //import var instance : CBehTreeInstance;
  
}

import abstract class CGameplayEntity extends CPeristentEntity {}
...
import abstract class CPlayer extends CActor {}
import abstract class CNewNPC extends CActor {
...
import abstract class CGame extends CObject {}
import abstract class CCommonGame extends CGame {
  import public var player : CPlayer;
}
import abstract class CR4Game extends CCommonGame {}
...
exec function mykillall(optional range: Float) {
  ...
  HudConsoleMsg("got enemies tst " + enemies.Size());
	...
	for (i = 0; i < enemiesSize; i+=1) {
	  ...
	  if (npc && npc.GetAttitude(theGame.player) == AIA_Hostile) {
	    npc.Kill(true,,);
    }
  }
}